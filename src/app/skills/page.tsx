...
import { CodeIcon, FolderIcon, FolderPlusIcon, ImageIcon, InfoIcon, LinkIcon, SparklesIcon, StarIcon } from 'lucide-react'
...
function SkillsPage() {
    ...
    const fetchKeySkills = async () => {
        try {
            ...
            if (docSnap.exists()) {
                setKeySkills(docSnap.data().keySkills || [])
            }
        } catch (error) {
            console.error('Failed to fetch key skills:', error)
            toast({
                title: "Error",
                description: "Failed to load key skills",
                variant: "destructive"
            })
        }
    }
    ...
    const addKeySkill = async () => {
        ...
        setIsLoading(true)
        try {
            const newKeySkillObj = {
                name: newKeySkill.name,
                imageUrl: newKeySkill.imageUrl
            }
            await updateDoc(keySkillsRef, {
                keySkills: arrayUnion(newKeySkillObj)
            })
            setKeySkills([...keySkills, newKeySkillObj])
            setNewKeySkill({ name: '', imageUrl: '' })
            toast({
                title: "Success",
                description: "Key skill added successfully"
            })
        } catch (error) {
            console.error('Failed to add key skill:', error)
            toast({
                title: "Error",
                description: "Failed to add key skill",
                variant: "destructive"
            })
        } finally {
            setIsLoading(false)
    ...
    const removeKeySkill = async (skillToRemove: KeySkill) => {
        ...
        try {
            await updateDoc(keySkillsRef, {
                keySkills: arrayRemove(skillToRemove)
            })
            setKeySkills(keySkills.filter(s => s.name !== skillToRemove.name))
            toast({
                title: "Success",
                description: "Key skill removed successfully"
            })
        } catch (error) {
            console.error('Failed to remove key skill:', error)
            toast({
                title: "Error",
                description: "Failed to remove key skill",
                variant: "destructive"
            })
        } finally {
            setIsLoading(false)
    ...
    const addCategory = async () => {
        ...
        try {
            ...
            await updateDoc(skillsRef, {
                categories: arrayUnion(newCategoryObj)
            })
    ...
    const addSkill = async () => {
        ...
        if (category?.skills.some(s => s.name === newSkill.name.trim())) {
            toast({
                title: "Error",
                description: "Skill already exists in this category",
                variant: "destructive"
            })
            return
        }
        ...
        try {
            const updatedCategories = categories.map(category => {
                if (category.name === selectedCategory) {
                    return {
                        ...category,
                        skills: [...category.skills, {...newSkill, updatedAt: new Date().toISOString()}]
                    }
                }
                return category
            })
    ...
    return (
        <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="space-y-4 max-w-7xl mx-auto p-4"
        >
                ...
                <CardContent>
                    <div className="flex flex-col space-y-2 mb-4">
                        <div className="flex items-center gap-2">
                            ...
                            <Input
                                placeholder="Add key skill name"
                                value={newKeySkill.name}
                                onChange={(e) => setNewKeySkill({ ...newKeySkill, name: e.target.value })}
                                disabled={isLoading}
                                className="bg-white/80 dark:bg-gray-950/60"
                            />
                        </div>
                        <div className="flex space-x-2">
                            <div className="flex items-center gap-2 flex-1">
                                ...
                                <Input
                                    placeholder="Add skill image URL"
                                    value={newKeySkill.imageUrl}
                    ...
                    <div className="flex flex-wrap gap-2">
                        {keySkills.length === 0 ? (
                            ...
                            keySkills.map((skill, index) => (
                                <div
                                    key={index}
                                    className="flex items-center gap-1 bg-teal-50 dark:bg-teal-900/20 text-teal-700 dark:text-teal-300 px-3 py-1 rounded-lg border border-teal-200 dark:border-teal-800"
                                >
                                    {skill.imageUrl && (
                                        <img 
                                            src={skill.imageUrl} 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <Card className="bg-gradient-to-br from-teal-50/30 to-teal-50/30 dark:from-teal-950/30 dark:to-teal-950/30 border-teal-100 dark:border-teal-900">
                    <CardHeader className="pb-2">
                        <div className="flex items-center gap-2">
                            <GearIcon className="h-5 w-5 text-teal-500" />
                            <CardTitle>Add New Skill</CardTitle>
                        </div>
                        <CardDescription>Add a skill to an existing category with proficiency level</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="flex flex-col space-y-2">
                            ...
                            >
                                <SelectTrigger className="bg-white/80 dark:bg-gray-950/60">
                            ...
                            <div className="flex space-x-2">
                                ...
                                <Input
                                    ...
                                    disabled={isLoading || !selectedCategory}
                                    className="w-24 bg-white/80 dark:bg-gray-950/80"
                                />
                                <Button 
                                    onClick={addSkill} 
                                    disabled={isLoading || !selectedCategory} 
                                    variant="outline"
                                    className="border-teal-500 hover:bg-teal-50 dark:hover:bg-teal-950"
                                >
                                    <PlusIcon className="h-4 w-4 text-teal-500" />
                                </Button>
                            </div>
                                ...
                                <AlertDialog>
                                    ...
                                    <AlertDialogContent>
                                        <AlertDialogHeader>
                                            ...
                                            <AlertDialogDescription>
                                                Are you sure you want to delete the category "{category.name}" and all its skills?
                                            </AlertDialogDescription>
                                        </AlertDialogHeader>
                                        <AlertDialogFooter>
                                    ...
                                    category.skills?.map((skill) => (
                                        <div key={skill.name} className="group">
                                            <div className="flex items-center justify-between mb-1.5">
                                                <div className="flex items-center gap-2">
                                                    <CodeIcon className="h-3 w-3 text-teal-500" />
                                                    ...
                                                    <AlertDialog>
                                                        ...
                                                        <AlertDialogContent>
                                                            ...
                                                            <AlertDialogFooter>
                                                                <AlertDialogCancel>Cancel</AlertDialogCancel>
                                                                <AlertDialogAction 
                                                                    onClick={() => removeSkill(category.name, skill.name)}
                                                                    className="bg-red-500 hover:bg-red-600 text-white"
                                                                >
                                                                    Delete
                                                                </AlertDialogAction>
